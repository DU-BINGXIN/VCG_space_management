...
def getIndexesNovalue(p, p_report):
    """ Get index positions of value in dataframe."""
    listOfPos = list()
    # Get bool dataframe with True at positions where the given value exists
    result = p.isin([p_report])
...
for z in range(10000):
    ...
    for i in range(consumer):
        ...
        pi_value = pi_value_list[0]
        pi_report = [i + 1,] + getIndexesNovalue(pi, pi_value)[0]
        table_no_value=table_no_value+[pi_report]
    for j in range(100):
        for re in table_no_value:
            report_table_part = re + [random.randint(0, 20)]
            report_table.append(report_table_part)
